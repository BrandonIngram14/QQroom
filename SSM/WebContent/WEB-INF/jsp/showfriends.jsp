		...
		<div class="w960  clf">
							...
							<div id="menuWrap" class="menuWrap" style="height: 140px;">
								<ul id="menu">
									...
									<ul id="child1" style="DISPLAY: none">
										<c:if test="${!empty  friends}">
											<c:forEach items="${friends}" var="friend" varStatus="status">
												<li>
												<a href="findFriend?friendid=${friend.friendid}" target="iframepage"><img
														src="static/headpic/${friend.user.headpic}" width="25" height="25" />${friend.user.username}</a>
												</li>
												</li>
											</c:forEach>
										</c:if>
									...
									<ul id="child2" style="DISPLAY: none">
										<c:if test="${!empty  lahei}">
											<c:forEach items="${lahei}" var="deletefriend"
												varStatus="s">
												<li>
											 <a href="findFriend?friendid=${deletefriend.friendid}" target="iframepage">
												<img src="static/headpic/${deletefriend.user.headpic }" width="25"
														height="25" />${deletefriend.user.username}</a>
												</li>
											</c:forEach>
										</c:if>
									</ul>
									...
									<ul id="child3" style="DISPLAY: none">
										<c:if test="${!empty  qingqiu}">
											<c:forEach items="${qingqiu}" var="waitfriend"
												varStatus="s">
												<li>
												 <a href="findFriend?friendid=${waitfriend.friendid}" target="iframepage">
												<img src="static/headpic/${waitfriend.user.headpic }" width="25" height="25" />${waitfriend.user.username}</a>
												</li>
											</c:forEach>
										</c:if>
									</ul>
			...
			<div class="m_main">
				<iframe frameborder=0 width=840 id="iframepage" name="iframepage"
					onLoad="iFrameHeight()" 
					src="tz?name=askforfriend"></iframe>
			</div>
		</div>
	</div>
