...
<body>
				<div class="inner">
					<div class="hd">
						...
						<a href="blackFriend?friendid=${friend.friendid}" class="comfirm" target="iframepage" onclick="return operaconfirm('拉黑');">拉黑</a>
						<a href="deleteFriend?friendid=${friend.friendid}" class="comfirm" target="iframepage" onclick="return operaconfirm('删除');">删除</a>
						</c:if>
						<c:if test="${friend.state==2 }">
						  <a href="backFriend?friendid=${friend.friendid }" class="comfirm" target="iframepage" onclick="return operaconfirm('恢复好友');" >恢复</a>
					      <a href="deletefriend?friendid=${friend.friendid}" class="comfirm" target="iframepage"  onclick="return operaconfirm('删除');">删除</a>
						</c:if>
						<c:if test="${friend.state==0 }">
						   <a href="noFriend?userid=${friend.friendid}" class="comfirm" target="iframepage"  onclick="return operaconfirm('拒绝');">拒绝</a>
						   <a href="okFriend?friendid=${friend.friendid}" class="comfirm" target="iframepage">同意</a>
						</c:if>
						</h3>
					</div>
					<div class="infos">
				...
				<div class="inner">
					...
					<div class="infos">
					 	QQ状态：
						<c:if test="${friend.user.state==1}">正常好友</c:if>
						<c:if test="${friend.user.state==0}">未添加好友</c:if> 
						<c:if test="${friend.user.state==2}">拉黑好友</c:if> 
						<br>  
						最近登录时间：${friend.user.lasttime}<br>
					</div>
				</div>
				</c:if>
